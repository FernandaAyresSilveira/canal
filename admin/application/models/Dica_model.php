<?php
	...
	class Dica_model extends  CI_model{
	    ...
	    public function get($id = false)
		{
			...
			if($id)
			{
				$query  = $this->db->get_where('dica', array('id'=> $id), 1);
				$result = $query->result();
				$result = $result[0];

			}
			else{
				$query  = $this->db->from('dica');
				$result = $query->result();
			}
		...
		public function get_paginate($p = null, $i = null){
			...
			if( $this->input->get('q') ){
				...
				$this->count = $this->db->count_all_results('dica');
		...
		public function save(){
			...
			$query = $this->db->insert('dica', $dados);
		...
		public function update(){
			...
			return $this->db->update('dica', $dados, $where);
		}
