<?php 
	...
	class Pedidos extends MY_Controller {
		...
		public function listar() {	
			...
			$config['base_url'] 			= $this->input->get() ? site_url('pedidos/listar').'?cliente='.$this->input->get('cliente').'&status='.$this->input->get('status') : site_url('pedidos/listar?g=1');
...
///////////// situação pagseguro ///////////////
		...
		{		
			...
			$escolhido = $novo_status;

			if ($novo_status=='cancelar_sememail') {
				$novo_status =6;
			}
					...
					<table cellpadding='0' cellspacing='0' width='800px' border='0' style='border-color: #fffff !important'>
					   ...
					   <table border='0' cellpadding='0' cellspacing='0' width='100%'>
					    ...
					    <tr>
					     <td style='padding: 25px 0 0 0;font-family:Arial;line-height:160%'>
					      ...
					      <center>Olá ".$venda->cliente_nome.". 
					      Seu pedido número ".$venda->id." foi cancelado pois não recebemos seu pagamento. Evite novos cancelamentos e a suspensão da sua conta em nossa loja, só faça seus pedidos quando tiver certeza que irá concluir o pagamento. Se tiver algum problema em suas compras entre em contato conosco, estamos aqui para lhe ajudar.</br>
									     </td>
					    </tr>
					   </table>
					  </td>
					 <tr>
					  <td width='260' valign='top' style='padding:0px 15px;font-family:Arial;line-height:160%'>
				   	...
				   	enviar_email('Seu pedido foi cancelado',$mensagem,$venda->cliente_email);
				}//fim de pago, em preparacao
		...
		public function listar_nao_finalizados() {	

			$i = 20;//Itens por página
			$p = $this->input->get('p') ? $this->input->get('p') : 1;//Página
			
			$data['pedidos'] = $this->pedido->get_paginate_nao_finalizados($p, $i);
			
			$this->load->library('pagination');
			$config['base_url'] 			= $this->input->get() ? site_url('pedidos/listar_nao_finalizados').'?cliente='.$this->input->get('cliente').'&status='.$this->input->get('status') : site_url('pedidos/listar_nao_finalizados?g=1');
			$config['total_rows'] 			= $this->pedido->count;
			$config['per_page'] 			= $i; 
			$config['first_link'] 			= '<<';
			$config['last_link']  			= '>>';
			...
			$this->load->view('estrutura/topo');
			$this->load->view('pedidos/listar_nao_finalizados', $data);
			$this->load->view('estrutura/rodape');
		}
		...
		public function embalagem() {
			$pedido = $this->input->get_post('id');

			echo $this->pedido->embalagem($pedido);

		}


		public function aviso_aguard_pag() {
			$pedido = $this->input->get_post('id');

			$retorno= $this->pedido->aviso_aguard_pag($pedido);

			$venda = $dados = $this->pedido->get($pedido);
			//var_dump($venda);

			if ($retorno == 1) {//vamos enviar o email
				$mensagem = "
					...
					<table cellpadding='0' cellspacing='0' width='800px' border='0' style='border-color: #fffff !important'>
					   ...
					   <table border='0' cellpadding='0' cellspacing='0' width='100%'>
					    ...
					    <tr>
					     <td style='padding: 25px 0 0 0;font-family:Arial;line-height:160%'>
					      ...
					      <center>Olá ".$venda->cliente_nome.".<br>
					       Ainda não identificamos seu pagamento para o pedido número ".$venda->id.". O prazo de pagamento se encerra amanhã, se estiver com alguma dúvida ou dificuldade entre em contato conosco e lhe ajudaremos.  </br> </br>
					      Caso tenha pago por boleto desconsidere esta mensagem. </br>
									     </td>
					    </tr>
					   </table>
					  </td>
					 <tr>
					  <td width='260' valign='top' style='padding:0px 15px;font-family:Arial;line-height:160%'>
				   	...
				   	enviar_email('Estamos aguardando seu pagamento',$mensagem,$venda->cliente_email);
			}
			echo $retorno;
